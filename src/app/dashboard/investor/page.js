...
export default function InvestorDashboard() {
  ...
  useEffect(() => {
    const fetchProfileAndInvoices = async () => {
      ...
      if (profileData?.role !== 'investor') {
         // ... (Keep your existing redirect logic here)
         router.push('/marketplace'); 
         return;
      }
      setProfile(profileData);

      // 2. Fetch Available Invoices
      // We explicitly select 'token_id' to ensure we have the blockchain ID
      ...
      // 3. Fetch Purchased Invoices (Invoices where the CURRENT user is the owner)
      // Since your Solidity contract transfers ownership, we can track this in Supabase too
      const { data: myInvestments } = await supabase
        .from('invoices')
        .select('*')
        .eq('status', 'Sold')
        .eq('owner', profileData.email); // Assuming you update 'owner' column on purchase
  ...
  const handlePurchase = async (invoice) => {
    ...
    // Check 2: Token ID Validity
    // IMPORTANT: We cannot buy it on-chain if we don't have the on-chain ID
    if (!invoice.token_id) {
        setMessage("Error: This invoice is missing its Blockchain Token ID.");
        return;
    }
    ...
    try {
      ...
      setMessage("Processing purchase... Please confirm transaction in MetaMask.");

      // 1. Execute Blockchain Transaction
      // We pass the token_id (e.g., "1") and the price (e.g., "0.5")
      const txReceipt = await purchaseInvoiceOnChain(invoice.token_id, invoice.listed_price);
      console.log("Purchase Receipt:", txReceipt);

      setMessage("Blockchain success! Updating database...");

      // 2. Update Database
      // We set status to 'Sold' and update the owner to the Investor
      const { error } = await supabase
        .from("invoices")
        .update({ 
          status: "Sold", 
          owner: profile.email, // Transfer ownership in DB
          owner_wallet: walletAddress,
          updated_at: new Date().toISOString()
        })
        .eq("id", invoice.id); // Use UUID to find the record in DB
      
      if (error) {
        setMessage("Blockchain payment worked, but DB update failed: " + error.message);
      } else {
        setMessage(`✅ Investment successful! You now own invoice #${invoice.invoice_number}`);
        // Refresh the lists
        setAvailableInvoices(prev => prev.filter(i => i.id !== invoice.id));
        setPurchasedInvoices(prev => [...prev, { ...invoice, status: 'Sold', owner: profile.email }]);
      }
    } catch (error) {
      console.error("Purchase failed:", error);
      let msg = "Transaction failed: ";
      if (error.message?.includes('User denied')) msg += "You rejected the transaction.";
      else if (error.message?.includes('insufficient funds')) msg += "Not enough ETH in your wallet.";
  ...
  // ... (Rest of your component, render return, and styles remain exactly the same)
  // Just ensure the button calls handlePurchase(invoice) instead of handlePurchase(id, price)
